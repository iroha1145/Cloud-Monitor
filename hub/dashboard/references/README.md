...
| 主题切换 | 09 icon swap | 日月图标交替 250ms；主题直接切换，避免整页视图过渡（View Transitions）期间浏览器屏蔽连续点击 |
...
只在用户操作或数据状态变化时触发。同步状态点在每次拿到新快照时闪一圈；数值没变时，数字和用量条不会重播动画。示例数据每次点「刷新数据」会给当天加一点用量，用来检查数字滚动和用量条过渡。减少动画设置下，页面区块、骨架脉动和抖动全部关闭，其余动效时长和延迟压到接近 0。
