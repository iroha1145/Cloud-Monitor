#!/usr/bin/env bash
# §13 e2e scratch 夹具：复制后端基线到临时目录，仅覆盖 hub/frontend/ 五个文件。
...
# 仅覆盖前端五个文件（测试夹具，非改后端）
cp "$FRONTEND_DIR/index.html" "$FRONTEND_DIR/tm.css" "$FRONTEND_DIR/tm.js" \
   "$FRONTEND_DIR/mock.js" "$FRONTEND_DIR/icons.svg" "$E2E_ROOT/hub/frontend/"
